...
# Purpose:        Runs regressions after dropping nodes
...
# filter 10 km
census <- census %>% filter(min_distance_to_node_km > 10)
grundtvig <- grundtvig %>% filter(min_distance_to_node_km > 10)
...
etable(tsls1$iv_first_stage[[1]], tsls2$iv_first_stage[[1]], tsls3$iv_first_stage[[1]],
       ...
       tex = TRUE,
       title = "First-Stage Estimates (Nodes dropped)")
...
nodes_dropped <- c("Nodes dropped (<10km)", "Yes", "Yes", "Yes", "Yes", "Yes")
